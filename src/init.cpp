 ...
 * between ratcoind, and ratcoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ratcoin/ratcoin>";
    const std::string URL_WEBSITE = "<https://ratcoin.net/>";
...
/** Sanity checks
 *  Ensure that ratcoin is running in a usable environment with all
...
{
    // Make sure only a single ratcoin process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // ratcoin: init hash seed
    ratcoinRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // ratcoin: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // ratcoin: checkpoint master priv key
    {
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if ratcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ratcoin is started while in a temporary directory.\n",
...
#ifdef ENABLE_CHECKPOINTS
                // ratcoin: initialize synchronized checkpoint
                if (!fReindex && !WriteSyncCheckpoint(chainparams.GenesisBlock().GetHash()))
                    return error("LoadBlockIndex() : failed to init sync checkpoint");

                // ratcoin: if checkpoint master key changed must reset sync-checkpoint
