...
#if defined(NDEBUG)
# error "ratcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "ratcoin Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // ratcoin: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
int64_t GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // ratcoin: subsidy is cut in half every 16x multiply of difficulty
...
// ratcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // ratcoin: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // ratcoin
            undo.nTime = alternate.nTime;           // ratcoin
        } else {
...
bool ratcoinContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    uint256 hashProofOfStake = uint256();
    // ratcoin: verify hash target and signature of coinstake tx
    ...
    // ratcoin: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // ratcoin: compute stake modifier
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !ratcoinContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // ratcoin: coinbase reward check relocated to CheckBlock()
    ...
    // ratcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // ratcoin: fees are not collected by miners as in bitcoin
    // ratcoin: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // ratcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // ratcoin: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // ratcoin: check transaction timestamp
...
#ifdef ENABLE_CHECKPOINTS
    // ratcoin: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // ratcoin: Don't reject in case of old clients. Change our assumption instead.
...
//    // ratcoin: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // ratcoin: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // ratcoin: check PoS
    if (fCheckPoS && !ratcoinContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // ratcoin: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // ratcoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // ratcoin: txindex is always enabled
...
// ratcoin: total coin age spent in transaction, in the unit of coin-days.
