...
// ratcoin: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    // ratcoin: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // ratcoin: there is no point to continue if we failed to create coinstake
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ratcoinMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet)
{
    ...
    RenameThread("ratcoin-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                ...
                LogPrintf("Error in ratcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            // ratcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
    ...
    {
        LogPrintf("ratcoinMiner terminated\n");
    return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ratcoinMiner runtime error: %s\n", e.what());
        return;
    }
}

// ratcoin: stake minter thread
...
// ratcoin: stake minter
void MintStake(boost::thread_group& threadGroup)
{
    // ratcoin: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
