...
// ratcoin: optional setting to unlock wallet for block minting only;
...
//    return n ? n : nTimeReceived;
    ...
    // ratcoin: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// ratcoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // ratcoin: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // ratcoin: set time
...
//                }
                ...
                // ratcoin: sub-cent change is moved to fee
...
// ratcoin: create coin stake transaction
